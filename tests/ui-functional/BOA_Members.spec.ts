...
test.describe('Edit BOA members scenarios in BOA details page', () => {
  ...
  test('Add Members in BOA', { tag: '@smoke' }, async ({ page }) => {
   ...
   test('Add same member different roles in BOA', { tag: '@smoke' }, async ({ page }) => {
    test.setTimeout(10 * 60 * 1000); // 10 minutes
    ...
    const WGrandomName = await groupActions.fillGroupNameFormWithRandomData('BOA');
