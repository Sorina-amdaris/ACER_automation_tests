...
test.describe('Edit AB members scenarios in AB details page', () => {
  ...
  test('Add Members in AB', { tag: '@smoke' }, async ({ page }) => {
    test.setTimeout(10 * 60 * 1000); // 10 minutes
    await groupActions.goto(manualData.addmembersInAB_URL);
    ...
    //   groupPage,
    //   groupActions,
    //   groupPage.removeConvenorButtonFromTableViewGrouPage,
    ...
    //   groupPage,
    //   groupActions,
    //   groupPage.removeExpertButtonFromTableViewGrouPage,
   ...
   test('Edit and Remove Members in AB', { tag: '@smoke' }, async ({ page }) => {
    test.setTimeout(10 * 60 * 1000); // 10 minutes
    await groupActions.goto(manualData.addmembersInAB_URL);
    await expect(groupPage.membersSection).toBeVisible();
    //switch to Table view so the remove-button presence checks below actually resolve
   ...
   test('Add same member different roles in AB', { tag: '@smoke' }, async ({ page }) => {
    test.setTimeout(10 * 60 * 1000); // 10 minutes
    ...
    const WGrandomName = await groupActions.fillGroupNameFormWithRandomData('AB');
